class Solution:
    def canFinish(self, numCourses: int, prerequisites: List[List[int]]) -> bool:
        ...
        for a, b in prerequisites:
            ...
            in_degree[a] += 1
            # only go from class before prerequisite to class that depends on it, so no need to add b to a's adjacency list here
            # only need to add a to b's adjacency list 
            adj[b].append(a)
