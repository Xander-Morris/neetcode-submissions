...
public:
    vector<vector<string>> groupAnagrams(vector<string>& strs) {
        ...
        for (const auto &pair : map) {
            ...
            pairs.emplace_back(strs[pair.first].size() == 0 ? "" : strs[pair.first]);
            ...
            for (const auto &other_pair : map) {
                ...
                pairs.emplace_back(strs[other_pair.first].size() == 0 ? "" : strs[other_pair.first]);
                in_anagram[other_pair.first] = true;
            }

            anagrams.emplace_back(pairs);
